...
class Queue: #A class for the queueing system
    ...
    def ValueIter(self,T=1,StartingState = [0,0]): #Recursively calculates cost for current state
       ...
       while not end:
                ...
                if e[0] == 0 or e[0] >= self.servers:
                    ...
                    if e[1] == 0:
                        ...
                        for i in self.A:
                            ...
                            ActionValues.append(self.Cost(e[0],i) + self.P2[1]*self.StateValues[T-1]['%s' %( [e[0]+i,0] )] + self.Pij(e[0],e[0])*self.StateValues[T-1]['%s' %([e[0],1])] + self.Pij(e[0],e[0]-1)*self.StateValues[T-1]['%s' %([e[0] + i - 1,1])] )
            ...
            RefVal =self.StateValues[T]['%s' %([0,1])]
       ...
       print self.StateValues[T - 1 ]['%s' %([0,1])]
       return self.P2[1]*self.StateValues[T - 1]['%s' %([0,0])],OptimalPolicy
       #return self.StateValues[T - 1],OptimalPolicy
'''
Simple = Queue(2,3,6,0.5,epsilon = 0.00000000001)
