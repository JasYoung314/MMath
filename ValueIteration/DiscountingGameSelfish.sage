            ...
            for e in self.states:
                ...
                if e[0] == 0 or e[0] >= self.servers:
                    if e[1] == 0 and not e[0] == self.Selfish:
                        ...
                        for i in self.A:
                            ActionValues.append((self.alpha**(T))*self.Cost(e[0],i) - i*self.R)

                        self.StateActions[T]['%s' %(e)] = ActionValues.index(min(ActionValues))
                        SA = ActionValues.index(min(ActionValues))

                        self.StateValues[T]['%s' %(e)] = (self.alpha**(T))*self.Cost(e[0],SA) - SA*self.R + self.P2[1]*self.StateValues[T-1]['%s' %( [e[0]+SA,0] )] + self.P2[0] *self.StateValues[T-1]['%s' %([max(e[0] + SA - 1,0),1])] - self.StateValues[T-1]['%s' %([0,0])]
                    ...
                    if e[1] == 0:
                        ...
                        self.StateValues[T]['%s' %(e)] = (self.alpha**(T))*self.Cost(e[0],SA) - SA*self.R + self.P2[1]*self.StateValues[T-1]['%s' %( [e[0]+SA,0] )] + self.Pij(e[0],e[0])*self.StateValues[T-1]['%s' %([e[0],1])] + self.Pij(e[0],e[0]-1)*self.StateValues[T-1]['%s' %([e[0] + SA - 1,1])] - self.StateValues[T-1]['%s' %([0,0])]
                    else:
...
Simple = Queue(2,3,6,0.5,epsilon = 0.000000000000001,alpha = 0.1)
