# This file was *autogenerated* from the file GameSelfish.sage.
        ...
        if not self.alpha**T < self.epsilon:
            ...
            if N >= self.servers or N == Integer(0):
                ...
                if not service:
                    ...
                    for e in self.A:
                        selfish[N].append((self.alpha**T)*(self.Cost(N,e)) +(Integer(1)-e)*self.R)

                    SelfishAction = selfish[N].index(min(selfish[N]))
                    values[N].append((( (self.alpha**T)*(self.Cost(N,SelfishAction)) +(Integer(1)- SelfishAction)*self.R + self.P2[Integer(0)]*self.SolveNHor(T+Integer(1),max(N+SelfishAction-Integer(1),Integer(0)),True))+self.P2[Integer(1)]*self.SolveNHor(T+Integer(1),N+SelfishAction)))
                ...
                if not service:
                    ...
                    SelfishAction = selfish[N].index(min(selfish[N]))
                    values[N].append((( (self.alpha**T)*(self.Cost(N,SelfishAction)) +(Integer(1)- SelfishAction)*self.R + self.Pij(N+SelfishAction,N+SelfishAction-Integer(1))*self.SolveNHor(T+Integer(1),N+SelfishAction-Integer(1),True)) + self.Pij(N + SelfishAction,N + SelfishAction)*self.SolveNHor(T + Integer(1), N + SelfishAction,True )  + self.P2[Integer(1)]*self.SolveNHor(T + Integer(1),N+SelfishAction)))
