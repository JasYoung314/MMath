        ...
        if not self.alpha**T < self.epsilon:
            ...
            if N >= self.servers or N == 0:
                ...
                if not service:
                    ...
                    for e in self.A[N]:
                        values[N].append((( (self.alpha**T)*(self.Cost(N,e)) + (1-e)*self.R + self.P2[0]*self.SolveNHor(T+1,max(N+e-1,0),True))+self.P2[1]*self.SolveNHor(T+1,N+e)))
                    if N in action:
                ...
                if not service:
                    ...
                    for e in self.A[N]:
                        values[N].append((( (self.alpha**T)*(self.Cost(N,e)) + (1-e)*self.R + self.Pij(N+e,N+e-1)*self.SolveNHor(T+1,N+e-1,True)) + self.Pij(N + e,N + e)*self.SolveNHor(T + 1, N + e,True )  + self.P2[1]*self.SolveNHor(T + 1,N+e)))
                    if N in action:
