    ...
    def Cost(self,state,action): # Returns the cost
        if action == Integer(0):
            ...
            if state < self.servers:
                return Integer(1)*((self.mu)**-Integer(1)) - self.R
            else:
                return (state+Integer(1))/((self.mu*self.servers)) - self.R
    @parallel
        ...
        if not self.alpha**T < self.epsilon:
            ...
            if N >= self.servers or N == Integer(0):
                ...
                if not service:
                    ...
                    for e in self.A[N]:
                        values[N].append((( (self.alpha**T)*(self.Cost(N,e))+self.P2[Integer(0)]*self.SolveNHor(T+Integer(1),max(N+e-Integer(1),Integer(0)),True))+self.P2[Integer(1)]*self.SolveNHor(T+Integer(1),N+e)))
                    if N in action:
                ...
                if not service:
                    ...
                    for e in self.A[N]:
                        values[N].append((( (self.alpha**T)*(self.Cost(N,e)) + self.Pij(N+e,N+e-Integer(1))*self.SolveNHor(T+Integer(1),N+e-Integer(1),True)) + self.Pij(N + e,N + e)*self.SolveNHor(T + Integer(1), N + e,True )  + self.P2[Integer(1)]*self.SolveNHor(T + Integer(1),N+e)))
                    if N in action:
            ...
            return min(values[N])
Simple = Queue(Integer(4),Integer(5),Integer(2),RealNumber('0.5'),alpha = RealNumber('0.6'),epsilon = RealNumber('0.01'))
A = Simple.SolveNHor(Integer(0),Integer(0))
for e in action:
    print e, action[e]
